...
| M4C Run Room | **CURRENT, SOURCE GATE OPEN** | Owner listing; recorded cost, hold, unknown usage, and pricing version are distinct. Start analysis sends explicit owner inputs through the readonly policy and run RPCs. A failed provider attempt shows safe provider metadata behind **Technical details**. | The browser sends only `{ runId, invocationKey }` to the Edge Function. It never writes runtime, reservation, or diagnostic tables. |
...
- **One request per attempt.** The SDK client is created with `maxRetries: 0`, and the injected transport refuses a second fetch before it reaches the network. The deadline covers headers and body. Base URL, organization, project, admin key, and logging are explicit, so no ambient environment variable redirects the request, adds a credential, or turns on logging. `OPENAI_CUSTOM_HEADERS` must not be set on the function; the SDK still reads it.
