-- 0011_document_records.sql
-- Add the private, metadata-only document archive record type.
-- Full document bodies remain in private Storage; record_data contains only
-- bounded metadata, extracted insight references, and paths to stored objects.

-- ----------------------------------------------------------------------
-- 1. Add document to the existing record type contract.
-- ----------------------------------------------------------------------
alter table public.records
  drop constraint if exists records_type_ck;

alter table public.records
  add constraint records_type_ck
  check (record_type in ('tool', 'repository', 'conversation', 'decision', 'document'));

-- ----------------------------------------------------------------------
-- 2. Strict document record_data validation.
--    The four existing branches from 0008 remain unchanged below this
--    document branch. Document objects intentionally reject all other keys,
--    including full-body/content fields.
-- ----------------------------------------------------------------------
  ...
  if rec_type = 'tool' then
    ...
    if pg_catalog.jsonb_typeof(data -> 'projectRoute') not in ('null','string')
       or (pg_catalog.jsonb_typeof(data -> 'projectRoute') = 'string'
           and (data ->> 'projectRoute') not in (
             'The Forge','The Chamber','The Book','General','Do not preserve'
           )) then
      raise exception 'document.projectRoute must be null or a valid route' using errcode = '22023';
    end if;
...
-- A protected RPC is still fed by caller-controlled JSON. This trigger binds
-- every stored document path to the row's owner and record id, so an own-user
-- path from another document cannot be smuggled through the generic Save RPC.
create or replace function public.validate_document_storage_paths()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  ...
  foreach path_key in array array['storagePath','extractedContentPath'] loop
    path_value := new.record_data ->> path_key;
    if path_value is not null and (
      pg_catalog.split_part(path_value, '/', 1) <> new.user_id::text
      or pg_catalog.split_part(path_value, '/', 2) <> 'documents'
      or pg_catalog.split_part(path_value, '/', 3) <> new.id::text
      or pg_catalog.split_part(path_value, '/', 4) not in ('original','extracted')
      or path_value !~ '^[0-9a-f-]{36}/documents/[0-9a-f-]{36}/(original|extracted)/[A-Za-z0-9][A-Za-z0-9._-]{0,254}$'
    ) then
      raise exception 'document.% must be scoped to its owner and record id', path_key using errcode = '22023';
    end if;
  end loop;
  return new;
end;
...
drop trigger if exists validate_document_storage_paths on public.records;
create trigger validate_document_storage_paths
before insert or update of record_type, user_id, id, record_data on public.records
for each row execute function public.validate_document_storage_paths();

revoke execute on function public.validate_document_storage_paths() from public;
revoke execute on function public.validate_document_storage_paths() from anon;
revoke execute on function public.validate_document_storage_paths() from authenticated;

-- ----------------------------------------------------------------------
-- 3. Keep the hardened 0008 save/restore implementations intact while
-- extending only their approved record-type lists. The source definitions
-- retain SECURITY DEFINER, empty search_path, grants, owner/link checks,
-- immutable stored record_type, dedicated old references, and preflight.
-- ----------------------------------------------------------------------
do $$
declare
  function_sql text;
  old_types constant text := '(''tool'',''repository'',''conversation'',''decision'')';
  new_types constant text := '(''tool'',''repository'',''conversation'',''decision'',''document'')';
begin
  function_sql := pg_catalog.pg_get_functiondef(
    'public.save_record_with_links(jsonb, uuid[])'::pg_catalog.regprocedure
  );
  if pg_catalog.strpos(function_sql, old_types) = 0 then
    raise exception 'save_record_with_links does not match the expected 0008 hardened definition';
  end if;
  execute pg_catalog.replace(function_sql, old_types, new_types);

  function_sql := pg_catalog.pg_get_functiondef(
    'public.restore_user_archive(jsonb)'::pg_catalog.regprocedure
  );
  if pg_catalog.strpos(function_sql, old_types) = 0 then
    raise exception 'restore_user_archive does not match the expected 0008 hardened definition';
  end if;
  execute pg_catalog.replace(function_sql, old_types, new_types);
end;
$$;
...
-- ----------------------------------------------------------------------
-- 4. Private Storage bucket and owner-scoped object policies.
--    Storage paths are exactly <user UUID>/documents/<record UUID>/<kind>/<safe file name>.
-- ----------------------------------------------------------------------
...
values (
  ...
  false,
  10000000,
  array['application/pdf','text/markdown','text/plain','application/json']::text[]
)
...
create policy document_files_select_own on storage.objects
  for select
  to authenticated
  using (
    ...
    and pg_catalog.array_length(storage.foldername(name), 1) = 4
    and (storage.foldername(name))[4] in ('original','extracted')
    and name ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/documents/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/(original|extracted)/[A-Za-z0-9][A-Za-z0-9._-]{0,254}$'
  );

drop policy if exists document_files_insert_own on storage.objects;
create policy document_files_insert_own on storage.objects
  for insert
  to authenticated
  with check (
    bucket_id = 'document-files'
...
create policy document_files_update_own on storage.objects
  for update
  to authenticated
  using (
    bucket_id = 'document-files'
...
create policy document_files_delete_own on storage.objects
  for delete
  to authenticated
  using (
    bucket_id = 'document-files'
