...
create or replace function public.purge_owner_account_data()
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('purge-owner-account:' || caller_id::text, 0)
  );

  -- Case-scoped Custodian graph (children cascade from cases where defined).
  delete from public.cases c where c.owner_id = caller_id;
  get diagnostics deleted_cases = row_count;

  delete from public.inbox_items i where i.owner_id = caller_id;

  -- Tables that may not sit under cases but still reference the owner via
  -- owner_id (and NO ACTION created_by/updated_by). Delete the rows so a later
  -- auth.users delete is not blocked.
  delete from public.audit_events ae where ae.owner_id = caller_id;
  delete from public.agent_provider_diagnostics d where d.owner_id = caller_id;
  delete from public.agent_provider_reservations r where r.owner_id = caller_id;
  delete from public.tool_events te where te.owner_id = caller_id;
  delete from public.agent_steps s where s.owner_id = caller_id;
  delete from public.approval_requests ar where ar.owner_id = caller_id;
  delete from public.change_proposals cp where cp.owner_id = caller_id;
  delete from public.automation_runs aur where aur.owner_id = caller_id;
  delete from public.automation_rules aul where aul.owner_id = caller_id;
  delete from public.agent_runs ag where ag.owner_id = caller_id;
  delete from public.tool_policies tp where tp.owner_id = caller_id;
  delete from public.connector_tools ct where ct.owner_id = caller_id;
  delete from public.connector_accounts ca where ca.owner_id = caller_id;
  delete from public.record_embeddings re where re.owner_id = caller_id;
  delete from public.forecasts fo where fo.owner_id = caller_id;
  delete from public.decision_reviews dr where dr.owner_id = caller_id;
  delete from public.experiments ex where ex.owner_id = caller_id;
  delete from public.entity_relations er where er.owner_id = caller_id;
  delete from public.entities en where en.owner_id = caller_id;
  delete from public.custodian_finding_evidence cfe where cfe.owner_id = caller_id;
  delete from public.custodian_findings cf where cf.owner_id = caller_id;
  delete from public.claim_evidence ce where ce.owner_id = caller_id;
  delete from public.claims cl where cl.owner_id = caller_id;
  delete from public.evidence_items ei where ei.owner_id = caller_id;
  delete from public.actions ac where ac.owner_id = caller_id;
  delete from public.case_members cm where cm.owner_id = caller_id;
  delete from public.record_revisions rr where rr.owner_id = caller_id;

  delete from public.record_links l where l.user_id = caller_id;
  get diagnostics deleted_links = row_count;

  delete from public.records r where r.user_id = caller_id;
  get diagnostics deleted_records = row_count;

  delete from public.app_metadata m where m.user_id = caller_id;
  delete from public.profiles p where p.id = caller_id;

  delete from public.owner_provider_credential_events e where e.owner_id = caller_id;
  delete from public.owner_provider_credentials c where c.owner_id = caller_id;
  get diagnostics deleted_credentials = row_count;
  delete from public.owner_provider_settings s where s.owner_id = caller_id;

  delete from private.conversation_extraction_quota q where q.user_id = caller_id;
...
revoke execute on function public.purge_owner_account_data() from public;
revoke execute on function public.purge_owner_account_data() from anon;
grant execute on function public.purge_owner_account_data() to authenticated;
